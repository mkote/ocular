...
def main(n_comp, band_list, subject, max_depth, oacl_ranges=None, m=None):
    print 'Running with following args \n'
    print n_comp, band_list, max_depth, subject, oacl_ranges, m
    ...
    for train_index, test_index in zip(train_indexes, test_indexes):
        ...
        accuracy = evaluate_fold(_train, _test, max_depth, band_list, n_comp)
...
def evaluate_fold(train, test, max_depth, band_list, n_comp, seeds=(4, 8, 15, 16, 23, 42)):
    ...
    accuracies = []
    for seed in seeds:
        rf = RandomForestClassifier(n_estimators=len(band_list) * 4 * n_comp, random_state=seed, bootstrap=False, max_depth=max_depth)
...
def evaluate(n_comp, band_list, max_depth, subject, oacl_ranges=None, m=None):
    old_path = os.getcwd()
    os.chdir('..')
    ...
    accuracy = evaluate_fold(train, test, band_list, max_depth, n_comp)
    print("Accuracy: " + str(accuracy * 100) + "%")
...
def translate_params(par):
    ...
    return n_comp, band_list, max_depth, oacl_ranges, m
...
if __name__ == '__main__':
    ...
    errors = []
    if len(sys.argv) > 1:
        n_comp, band_list, max_depth, oacl_ranges, m = translate_params(sys.argv[2:])
