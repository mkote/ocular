...
When Bayesian optimization builds its model of the objective function, it iteratively choses inputs to sample outputs from the objective function. The choice of which inputs to sample from is determined by the \emph{acquisition function} (AF), which determines utility of sampling from a given input. Different acquisition functions yield different measures to make this decision. One measure is the \emph{Probability of Improvement} (PI) which given a candidate input computes the probability of improving the current best result. Another approach is to consider the \emph{expected improvement} (EI) which not only takes into account the probability of improvement, but also the uncertainty e.g. the variance of the Gaussian distribution of the surrogate at the given input. \todo{Læs Herfra}The bayesian optimization of a dummy function can be seen in \cref{fig:bayesian-optimization}. The objective function, together with a GP and the corresponding surrogate function can be seen. We also see the EI of probing the surrogate function, and the next parameter to probe the objective function, which will be the place with maximum expected improvement, when using EI as the acquisition function.
\todo{Læs Hertil}
