...
\end{equation}
where $\pmb{I}$ is the identity matrix. The variance accounted for by the first m eigenvectors in $\pmb{D}$ will maximize for class A and the m lass will be for class B. New trial can then be transformed by
\begin{equation}
\pmb{Z} = \pmb{X} \cdot \pmb{D}
\end{equation}
