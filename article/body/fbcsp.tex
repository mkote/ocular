...
\end{equation}
where $\pmb{I}$ is the identity matrix. The eigenvectors in $\pmb{P}$ are ordered such that the variance of the first is maximized for class A, and the variance of the last is maximized for class B. The new trial can then be transformed by
\begin{equation}
\pmb{Z} = \pmb{X} \cdot \pmb{P}
\end{equation}
where $\pmb{X}$ are are a trial, and $\pmb{Z}$ is its transformation.
